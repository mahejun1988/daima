...
public class LoginController {
	...
	public InvokeResult login(HttpServletRequest request, LoginCommand command) {
		...
		if(!invokeResult.isSuccess()){
			...
			try {
				...
				return InvokeResult.failure("login.error1");			
			} catch (LockedAccountException e) {
				LOGGER.error(e.getMessage(), e);
				return InvokeResult.failure("login.error2");			
			} catch (AuthenticationException e) {
				LOGGER.error(e.getMessage(), e);
				return InvokeResult.failure("login.error3");
			} catch (Exception e) {
				LOGGER.error(e.getMessage(), e);
				return InvokeResult.failure("login.error4");
			}
		}
	}
	...
	private InvokeResult doCaptcha(HttpServletRequest request) {
		...
		if (!StringUtils.isBlank(shiroLoginFailure)) {
			return	InvokeResult.failure("login.error5");
		}
		return InvokeResult.success();
	}
}
