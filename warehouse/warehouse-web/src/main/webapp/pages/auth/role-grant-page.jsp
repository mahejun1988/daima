...
$(function(){
    ...
    var url = contextPath + "/auth/role/pagingQueryGrantPageElementResourcesByRoleId.do?roleId="+ pageId;
    ...
    $('[data-role="roleGrantPageGrid"]').grid({
        ...
        "assignPageFromRole" : function(event, data){
            ...
            $.get(contextPath + '/pages/auth/page-select.jsp').done(function(data){
                ...
                dialog.find('#save').click(function(){
                    ...
                    $.post(contextPath + '/auth/role/grantPageElementResourcesToRole.do', data).done(function(data){
                        if(data.success){
                            grid.message({
                                type: 'success',
                                content: '保存成功'
                    ...
                    'shown.bs.modal': function(){ //弹窗初始化完毕后，初始化url选择表格
                        ...
                        dialog.find('#selectPageGrid').grid({
                            identity: 'id',
                            columns: columns,
                            url: contextPath + '/auth/role/pagingQueryNotGrantPageElementResourcesByRoleId.do?roleId='+pageId
                        });
        ...
        'removePageFromRole' : function(event, data) {
            ...
            grid.confirm({
                content : '确定要删除所选记录吗?',
                callBack : function() {
                    var url = contextPath + '/auth/role/terminatePageElementResourcesFromRole.do';
