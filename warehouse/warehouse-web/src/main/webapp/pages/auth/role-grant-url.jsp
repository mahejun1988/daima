...
$(function() {
    ...
    url = contextPath + '/auth/role/pagingQueryGrantUrlAccessResourcesByRoleId.do' + '?roleId=' + roleId;
    ...
    $('[data-role="roleGrantUrlGrid"]').grid({
        ...
        "assignUrl" : function(event, data){
            ...
            $.get(contextPath + '/pages/auth/url-select.jsp').done(function(data){
                ...
                dialog.find('#save').click(function(){
                    ...
                    $.post(contextPath + '/auth/role/grantUrlAccessResourcesToRole.do', data).done(function(data){
                        grid.message({
                            type: 'success',
                            content: '保存成功'
                        });
                    ...
                    'shown.bs.modal': function(){ //弹窗初始化完毕后，初始化url选择表格
                        ...
                        dialog.find('#selectUrlGrid').grid({
                            identity: 'id',
                            columns: columns,
                            url: contextPath + '/auth/role/pagingQueryNotGrantUrlAccessResourcesByRoleId.do?roleId=' + roleId
                        });
                    },
        ...
        "removeUrlFromRole" : function(event, data){ //解除授予
            ...
            grid.confirm({
                content : '确定要删除所选记录吗?',
                callBack : function() {
                    var url = contextPath + '/auth/role/terminateUrlAccessResourcesFromRole.do';
