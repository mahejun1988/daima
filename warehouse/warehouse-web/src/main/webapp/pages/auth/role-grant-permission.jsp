...
$(function(){
    ...
    var url = contextPath + '/auth/role/pagingQueryGrantPermissionsByRoleId.do?roleId=' + roleId;
    ...
    $('[data-role="roleGrantPermissionGrid"]').grid({
        ...
        'assignPermissionForRole':function(event, data){
            ...
            $.get(contextPath + '/pages/auth/permission-select.jsp').done(function(data){
                ...
                dialog.find('#save').click(function(){
                    ...
                    $.post(contextPath + '/auth/role/grantPermissionsToRole.do', data).done(function(data){
                        if(data.success){
                            grid.message({
                                type: 'success',
                                content: '保存成功'
                    ...
                    'shown.bs.modal': function(){ //弹窗初始化完毕后，初始化权限选择表格
                        ...
                        dialog.find('.selectPermissionGrid').grid({
                            identity: 'id',
                            columns: columns,
                            url: contextPath + '/auth/role/pagingQueryNotGrantPermissionsByRoleId.do?roleId='+roleId
                        });
                    },
                    'complete': function(){
                        grid.message({
        ...
        'removePermissionForRole':function(event, data) {
            ...
            grid.confirm({
                content : '确定要删除所选记录吗?',
                callBack : function() {
                    var url = contextPath + '/auth/role/terminatePermissionsFromRole.do';
