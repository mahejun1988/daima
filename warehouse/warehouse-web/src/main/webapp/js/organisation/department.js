var department = function(){
		...
		dialog.find('#save').on('click',function(){
			...
			'complete': function(event, data){
                ...
                if(type == 'updateCompany' || type == 'updateDepartment'){
                	 $.get(contextPath + '/organization/get.do?id='+id).done(function(data){
                        var org = data;
                        $element.data(org);
                        if(!$element.hasClass('tree-item')){
	...
	var setData = function(id){
		$.get(baseUrl+'get.do?id='+id).done(function(data){
			org = data;
	...
	var getTree = function(id){
        ...
        $.get(baseUrl + 'org-tree.do').done(function(data){
		...
		grid.confirm({
			...
			callBack: function(){
				$.ajax({
					...
					'type': "Post",
					'url':  contextPath + '/organization/terminate-eo-relations.do?organizationId=' + id,
