var userManager = function() {
	...
	var resetPassword = function(item, grid) {
		var dataGrid = grid;
		console.log(item.id);
		$.post(contextPath + '/auth/user/resetPassword.do?userId=' + item.id).done(function(data) {
			if (data.success) {
				dataGrid.message({
					type : 'success',
	...
	var available = function(item, grid) {
		var dataGrid = grid;
		$.post(contextPath + '/auth/user/activate.do?userId=' + item.id).done(function(data) {
	...
	var forbidden = function(item, grid) {
		var dataGrid = grid;
	    console.log(item.id);
		$.post(contextPath + '/auth/user/suspend.do?userId=' + item.id).done(function(data) {
	...
	var deleteUser = function(users, grid) {
        ...
        dataGrid = grid;
		$.post(contextPath + '/auth/user/terminate.do', data).done(function(data){
			if(data.success){
				dataGrid.message({
					type: 'success',
					content: '撤销成功'
