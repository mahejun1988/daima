var roleManager = function(){
	...
	var assignResource = function(grid, roleId){
		$.get(contextPath + '/pages/auth/menu-select.jsp').done(function(data){
            ...
            dialog.find('.save').on('click',function(){
				...
				var url =baseUrl + 'grantMenuResourcesToRole.do?roleId=' + roleId;
	...
	* 加载资源树
	...
	var initResourceTree = function(dialog, roleId){
		$.get(contextPath + '/auth/role/findMenuResourceTreeSelectItemByRoleId.do?time='+new Date().getTime()+'&roleId='+roleId).done(function(result){
