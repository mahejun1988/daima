...
$(function(){
	...
	* 菜单收缩样式变化
	...
	self.on('modifyPwd',function(){
		$('body').modifyPassword({
			service: contextPath + '/auth/currentUser/updatePassword.do'
		});
	});
	...
	*/
	self.on('loginOut', function(){
		var logOut = contextPath+"/logout.do";
...
var loadContent = function(obj, url){
	$.get(contextPath + url).done(function(data, status, objXMLHttp){
    	...
    	if (headers.indexOf("login: login") >= 0 && window.location.pathname.indexOf("/login.do") < 0) {
    		window.location.href = contextPath + "/login.do";
    	} else {
    		obj.html(data);
